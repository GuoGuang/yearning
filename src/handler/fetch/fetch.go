...
func FetchTags(c yee.Context) (err error) {
	var tags []string
	model.DB().Model(model.CoreSqlOrder{}).Select("tag").Group("tag").Find(&tags)
	// Execute the query
	return c.JSON(http.StatusOK, common.SuccessPayload(tags))

}

func ChangeRecord(c yee.Context) (err error) {
	u := new(_FetchBind)
	...
	var results *Results
	url := "https://ykcloud.inca.com.cn/api/blade-system/timer/ddl"
	table := u.Table
	status := 1
	typ := 0
	sourceId := u.SourceId
	dataBase := u.DataBase
	if table == "" || sourceId == "" || dataBase == "" {
		return c.JSON(http.StatusOK, common.SuccessPayload(results))
	}

	model.DB().Table("core_sql_orders o").
		Select("o.type, o.real_name, o.execute_time, cqr.sql").
		Joins("join core_sql_records cqr on o.work_id=cqr.work_id").
		Where("o.table = ? AND o.status = ? AND o.type = ? AND o.source_id= ?  AND o.data_base= ? ", table, status, typ, sourceId, dataBase).
		Scan(&totals)

	postData, err := json.Marshal(totals)
	if err != nil {
